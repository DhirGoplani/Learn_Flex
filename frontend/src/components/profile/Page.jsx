...
const StatCard = ({ icon: Icon, label, value, subtext, colorClass, delay }) => (
  ...
  >
    ...
    <div className="relative z-10">
      <div className={`w-12 h-12 rounded-xl ${colorClass} bg-opacity-20 flex items-center justify-center mb-4 border border-white/10`}>
      ...
      <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-slideUp">
        ...
        <div className="p-6 space-y-6">
          <div className="space-y-4">
            <h3 className="text-sm font-medium text-slate-400 uppercase tracking-wider">Account</h3>
            <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-white/5 hover:border-violet-500/30 transition-colors cursor-pointer group">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-violet-500/10 rounded-lg text-violet-400"><User size={18} /></div>
                <div>
                  <div className="text-white font-medium">Edit Profile</div>
                  <div className="text-xs text-slate-500">Change name, avatar</div>
                </div>
              </div>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-medium text-slate-400 uppercase tracking-wider">Preferences</h3>
             <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-white/5 hover:border-violet-500/30 transition-colors cursor-pointer group">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-amber-500/10 rounded-lg text-amber-400"><Bell size={18} /></div>
                <div>
                  <div className="text-white font-medium">Notifications</div>
                  <div className="text-xs text-slate-500">Email digests, alerts</div>
                </div>
              </div>
    ...
    <div className="relative min-h-screen w-full font-sans text-slate-200 selection:bg-violet-500/30 overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-40 w-full bg-slate-900/70 backdrop-blur-xl border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold text-white tracking-tight hidden sm:block">QuizMaster</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-8 animate-fadeIn">
            <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 relative overflow-hidden group">
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                ...
                <div className="w-full bg-slate-800/50 rounded-xl p-4 border border-white/5 mb-6">
                  ...
                  <div className="text-right text-xs text-slate-500 mt-1">750 / 1000 XP</div>
                </div>
            ...
            <div className="bg-gradient-to-br from-indigo-900/40 to-slate-900/40 border border-indigo-500/20 rounded-2xl p-6 relative">
              <div className="text-indigo-400 mb-2"><Sparkles size={20} /></div>
              <p className="text-slate-300 italic text-sm">"Knowledge increases by sharing but not by saving."</p>
              <p className="text-slate-500 text-xs mt-2 font-medium">— Kamari aka Lyrikal</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 animate-fadeIn" style={{ animationDelay: '400ms' }}>
                ...
                <div className="flex gap-2">
                   {['Week', 'Month', 'Year'].map(tab => (
                     <button 
                       ...
                       className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${activeTab === tab.toLowerCase() ? 'bg-violet-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                     >
                       {tab}
                     </button>
                   ))}
              ...
              {/* Mock Activity Graph Visual */}
              <div className="h-48 w-full flex items-end justify-between gap-2 px-2">
                {[40, 65, 45, 80, 55, 90, 75, 60, 85, 95, 70, 65].map((h, i) => (
                  <div key={i} className="w-full bg-slate-800/50 rounded-t-lg relative group h-full flex items-end">
                    <div 
                      className="w-full bg-gradient-to-t from-violet-600/50 to-fuchsia-500/50 rounded-t-md hover:from-violet-500 hover:to-fuchsia-400 transition-all duration-300 relative"
                      style={{ height: `${h}%` }}
                    >
                      <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-white text-slate-900 text-xs font-bold py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        {h}
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4 animate-fadeIn" style={{ animationDelay: '500ms' }}>
               <div className="bg-gradient-to-r from-violet-900/40 to-slate-900/40 border border-violet-500/10 p-4 rounded-2xl flex items-center justify-between group cursor-pointer hover:border-violet-500/30 transition-all">
                  ...
                  <div className="w-10 h-10 rounded-full bg-violet-600/20 flex items-center justify-center group-hover:bg-violet-600 transition-colors">
                    <ChevronRight className="w-5 h-5 text-violet-400 group-hover:text-white" />
                  </div>
               </div>
               ...
               <div className="bg-gradient-to-r from-blue-900/40 to-slate-900/40 border border-blue-500/10 p-4 rounded-2xl flex items-center justify-between group cursor-pointer hover:border-blue-500/30 transition-all">
                ...
                <div className="bg-gradient-to-r from-emerald-900/40 to-slate-900/40 border border-emerald-500/10 p-4 rounded-2xl flex items-center justify-between group cursor-pointer hover:border-emerald-500/30 transition-all">
                  <div className="flex flex-col">
                    <span className="text-white font-medium">Ranked Quiz</span>
                    <span className="text-xs text-emerald-300">Compete globally</span>
                  </div>
                  <div className="w-10 h-10 rounded-full bg-emerald-600/20 flex items-center justify-center group-hover:bg-emerald-600 transition-colors">
